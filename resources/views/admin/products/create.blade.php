...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.products.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('size') ? 'is-invalid' : '' }}" name="size_id" id="size_id">
                    @foreach($sizes as $id => $size)
                        <option value="{{ $id }}" {{ old('size_id') == $id ? 'selected' : '' }}>{{ $size }}</option>
                    @endforeach
                </select>
                @if($errors->has('size'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.product.fields.featured') }}</label>
                <select class="form-control {{ $errors->has('featured') ? 'is-invalid' : '' }}" name="featured" id="featured" required>
                    ...
                    @foreach(App\Models\Product::FEATURED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('featured', 'featured') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
