    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.shipments.update", [$shipment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('area') ? 'is-invalid' : '' }}" name="area_id" id="area_id" required>
                    @foreach($areas as $id => $area)
                        <option value="{{ $id }}" {{ (old('area_id') ? old('area_id') : $shipment->area->id ?? '') == $id ? 'selected' : '' }}>{{ $area }}</option>
                    @endforeach
                </select>
                @if($errors->has('area'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shipment_company') ? 'is-invalid' : '' }}" name="shipment_company_id" id="shipment_company_id" required>
                    @foreach($shipment_companies as $id => $shipment_company)
                        <option value="{{ $id }}" {{ (old('shipment_company_id') ? old('shipment_company_id') : $shipment->shipment_company->id ?? '') == $id ? 'selected' : '' }}>{{ $shipment_company }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $shipment->price) }}" step="0.01" required>
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
